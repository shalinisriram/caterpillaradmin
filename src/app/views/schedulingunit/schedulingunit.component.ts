...
@Component({
  selector: 'app-schedulingunit',
  templateUrl: './schedulingunit.component.html',
  styleUrls: ['./schedulingunit.component.css']
})
export class SchedulingunitComponent implements OnInit {
  ...
  public form: FormGroup;
    ...
    displayedColumns2 = [ 'SerialNumber', 'SectionName', 'ModelName', 'AreaName', 'ComponentName','CreateTime', 'Select'];
   ...
   getSchedulingUnits()
   {
    ...
    showUpdateForm(element) {
        ...
        this.units = { ...element }
        this.units.sectionName = element.SectionName
        this.units.ModeName = element.ModeName
        this.units.AreaName = [element.AreaName]
        this.units.CreatedTimeStamp = element.CurrentTimeStamp
    ...
    UpdateAddunit()
    {
      this._unitService.EditSchedulingUnit(this.units).subscribe(
        (response)=>
        {
            const res = response;
            if(res.code)
    ...
    deleteUnits() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                this._unitService.deleteScheduleUnits(obj).subscribe(
                    (response) => {
                        const result = response;
                        if (result.code) {
                            Swal.fire(
